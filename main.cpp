...
private:  
    ...
    bool owns_input;            // True if we can delete p_input, false if we ccan't

    Token current_token;
    std::string current_token_text;

    void init();                    // Code common to each construxtor.
...
Token Lexer::get_token()
{
    ...
    if (isalpha(c)) {
        ...
        // Whatever is not a function name must be a identidier.
        return Token::Id;
    }
    ...
    if (isdigit(c)) {
        ...
        // Look for an optional exponents part.
        exponent_part(c);
    ...
    for (;;) {
        switch (p_lexer->get_current_token()) {
            ...
            case Token::Div:  
                ...
                x = pow_expr();
                if (x == 0) 
                    throw Runtime_error{"attemping to divide by zero"};
                result /= x;
                break;
            case Token::Mod:  
                p_lexer->advance();
...
double Parser::primary()
{
    ...
    switch (p_lexer->get_current_token()) {
        ...
        case Token::Sqrt:  
            arg = get_argument();
            if (arg < 0)
                throw Runtime_error{"attempt to make square root of negative number"};
            return sqrt(arg);
        case Token::Int:  
            arg = get_argument();
            if (arg < 0)
                ...
                return floor(arg);
        default:  
            throw Syntax_error{"invalid primary exxpression"};
        }
}
...
double Parser::get_argument()
{
    ...
    double arg = add_expr();
    if (p_lexer->get_current_token() != Token::Rp)
        throw Syntax_error{"misiing ) after function argument"};
    p_lexer->advance();
    return arg;
}
